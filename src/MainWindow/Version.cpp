...
void MainWindowPrivate::commitCurrentSnapshot() {
    ...
    const QString feedbackCommitHash = currentWorkContextHash();
    ...
    if (!newHead.isEmpty() && feedbackCommitHash != newHead) {
        QString errorMessage;
        if (!m_feedbackStore.reassignCommit(feedbackCommitHash, newHead, &errorMessage))
