...
namespace {
QStringList projectPathspecExcludingMetadata() {
    return {"--", ".", ":(exclude).hwpilot/**"};
}
}  // namespace
...
QString GitService::statusPorcelain() const {
    QStringList arguments = {"status", "--short", "--untracked-files=all"};
    arguments.append(projectPathspecExcludingMetadata());
    return runGit(arguments).stdoutText;
}
...
QString GitService::diffStat() const {
    QStringList arguments = {"diff", "--stat"};
    arguments.append(projectPathspecExcludingMetadata());
    return runGit(arguments).stdoutText;
}
...
QString GitService::diffForCommit(const QString& commitHash) const {
    ...
    QStringList arguments = {"show", "--format=", "--patch", commitHash};
    arguments.append(projectPathspecExcludingMetadata());
    return runGit(arguments).stdoutText;
}
...
QString GitService::diffStatForCommit(const QString& commitHash) const {
    ...
    QStringList arguments = {"show", "--format=", "--stat", commitHash};
    arguments.append(projectPathspecExcludingMetadata());
    return runGit(arguments).stdoutText;
}
...
QStringList GitService::changedPaths() const {
    QStringList arguments = {"status", "--porcelain=v1", "-z", "--untracked-files=all"};
    arguments.append(projectPathspecExcludingMetadata());
