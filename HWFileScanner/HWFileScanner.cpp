...
QList<CodeFile> HWFileScanner::scanDirectory(
    ...
    int maxFileSizeKB)
{
    ...
    while (it.hasNext()) {
        ...
        if (fileInfo.absolutePath().contains("/.")) continue;
        ...
        if (!isCMake && !isTargetFile(suffix, extensions)) continue;

        // 检查文件大小
        qint64 fileSizeKB = fileInfo.size() / 1024;
        if (fileSizeKB > maxFileSizeKB) {
            qDebug() << "[HWFileScanner] 跳过超大文件（"
                     << fileSizeKB << "KB）：" << fileInfo.fileName();
            continue;
        }
    ...
    // 按相对路径排序，保证结果稳定
    std::sort(result.begin(), result.end(), [](const CodeFile& a, const CodeFile& b) {
...
QString HWFileScanner::formatFilesForLLM(
...
void HWFileScanner::printScanSummary(const QList<CodeFile>& files, const QString& dirPath)
{
    ...
    for (int i = 0; i < files.size(); ++i) {
        ...
        qDebug() << QString("  [%1] %2  (%3 KB)")
                        .arg(i + 1)
                        .arg(f.relativePath)
                        .arg(sizeKB == 0 ? QString("<1") : QString::number(sizeKB));
    }
    qDebug() << "========================================\n";
}
